...
"""
function azimuth(e::Event, s::Station; sphere=false, flattening=Geodesics.F_WGS84)
...
then `flattening` is set to zero and the calculation is performed on a sphere.
"""
function backazimuth(s::Station, e::Event; sphere=false, flattening=Geodesics.F_WGS84)
    _check_headers_geometry(e, s)
...
then `flattening` is set to zero and the calculation is performed on a sphere.
"""
function distance_deg(e::Event, s::Station; sphere=false, flattening=Geodesics.F_WGS84)
    _check_headers_geometry(e, s)
...
"""
function distance_km(e::Event, s::Station; sphere=false, a=Geodesics.EARTH_R_MAJOR_WGS84/1e3,
